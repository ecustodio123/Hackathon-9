...
class Conexion:
    def __init__(self, table_name):
        ...
        self.db = connect(host='127.0.0.1', user='postgres', password='admin', database='minimarket')
    ...
    def where_like(self, data):
        query = f"SELECT * FROM {self.table_name} WHERE producto LIKE '%{data}%'"
        self.cursor.execute(query)
        return self.cursor.fetchone()

    def where_name(self, data):
        query = f"SELECT * FROM {self.table_name} WHERE nombre = '{data}'"
        self.cursor.execute(query)
        return self.cursor.fetchone()

    def where_dia(self, fecha):
        query = f"SELECT * FROM {self.table_name} WHERE fecha = '{fecha}'"
        self.cursor.execute(query)
        return self.cursor.fetchall()

    def where_mes(self, mes_venta):
        query = f"SELECT * FROM {self.table_name} WHERE mes_venta = '{mes_venta}'"
        self.cursor.execute(query)
        return self.cursor.fetchall()
